my $id = 'V-22473';
my $title = 'The SSH daemon must not permit GSSAPI authentication unless needed.';
my $severity = 'low';
...
my $fix = 'Edit /etc/ssh/sshd_config and remove the GSSAPIAuthentication setting or change the value to "no".';
my $autotest = 1;
my $autofix = 1;
...
sub fix()
{
    return STIG::sedi($filename, 's/^GSSAPIAuthentication/#&/');;
;
}
